			...
			if (!err) {
				err = AudioUnitSetProperty(
					...
					if (url) {
						...
						LOG_ERROR("MIDI: Failed to allocate CFURLRef from  {}",soundfont);
					}
#endif
					if (!err) {
						LOG_INFO("MIDI:coreaudio: loaded soundfont: {}",soundfont);
					} else {
						LOG_ERROR("MIDI: Error loading CoreAudio SoundFont {}",soundfont);
