...
void init_gus_dosbox_settings(Section_prop &secprop)
{
	...
	assert(bool_prop);
	bool_prop->Set_help(_("Enable Gravis UltraSound emulation."));
	...
	hex_prop->Set_help(_("The IO base address of the Gravis UltraSound."));
	...
	int_prop->Set_help(_("The IRQ number of the Gravis UltraSound."));

	int_prop = secprop.Add_int("gusdma", when_idle, 3);
	assert(int_prop);
	const char *const dmas[] = {"3", "0", "1", "5", "6", "7", nullptr};
	int_prop->Set_values(dmas);
	int_prop->Set_help(_("The DMA channel of the Gravis UltraSound."));

	auto *str_prop = secprop.Add_string("ultradir", when_idle, "C:\\ULTRASND");
	assert(str_prop);
	str_prop->Set_help(_("Path to UltraSound directory. In this directory\n"
